...
- [x] **Step 5: 수동 회귀 체크리스트** — 2026-07-28 레고가 새로 빌드한 앱(정렬 클릭 영역 버그 수정판, 0.9.416)으로 직접 확인 완료. 아래 3개(칼럼 드래그·마우스 호버 클릭·0건 문구)만 미확인 — 마우스 자동 조작 도구가 없어 GJC가 대신 못 함, 코드 경로상 문제 소지 낮음.

- [x] ⇧⌘O로 검색창 열기 → 빈 검색어에서 최근 파일이 표 형태(이름/경로/크기/수정일)로 보임(GJC 키보드 확인)
- [x] 검색어 입력 → 이름 유사도 결과가 같은 표 형태로 보임, 검색어 지우면 다시 최근 파일 표로 복귀(GJC 키보드 확인)
- [x] "이름"/"경로"/"크기"/"수정일" 헤더 클릭 → 각각 기준으로 정렬, 재클릭 시 방향 반전(레고 실기 확인, 0.9.416 재설치 후)
- [ ] 칼럼 경계 드래그로 폭 조절(미확인 — 마우스 드래그, GJC 도구로 불가)
- [x] 화살표 위/아래로 하이라이트 이동(파일 행 ↔ 본문 검색 행 경계 포함), 엔터로 하이라이트된 항목 열림(GJC 키보드 확인)
- [ ] 마우스 호버 시 하이라이트 이동, **클릭 한 번**으로 즉시 열림(더블클릭 아님)(미확인 — 마우스 클릭, GJC 도구로 불가. 단 로직은 엔터 열기와 같은 `open(at:in:)` 공유라 회귀 위험 낮음)
- [x] 2글자 이상 입력 시 본문 검색(In-file Matches) 결과가 지금처럼 줄 번호+스니펫 목록으로 보임(표로 안 바뀜)(GJC 확인 — "IN-FILE MATCHES" 섹션이 표와 별도로 렌더링됨)
- [ ] 검색 결과 0건일 때 안내 문구(`ContentUnavailableView`) 정상 표시(미확인 — 이 앱 검색이 퍼지 매칭이라 623,000여개 색인 중 완전히 안 걸리는 문자열을 못 만듦)
- [x] ESC로 창 닫기 정상 동작(GJC 확인)
...
- [x] `swift test` 전체 재실행, 기준선 대비 정확히 +6(Task 2 테스트) 확인 — 883 → 889.
- [x] `CmdMD-fork_prd.md` §10 "Docufinder 대비 기능 격차" 1번 항목에 완료 표기 추가(다른 완료 항목과 같은 형식).
- [ ] 레고에게 실제 화면(스크린샷 또는 실행 화면)으로 최종 확인받기 — 사람이 직접 앱을 눌러보는 확인은 이번 작업 지시에 따라 하지 않음. 레고 확인 대기.
