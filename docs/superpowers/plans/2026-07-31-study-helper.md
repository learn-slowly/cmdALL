...
- 상태: **S0 완료(2026-07-31)** — 기존 AI 패널에 "정리 카드 만들기"·"문제 뽑기" 프리셋 버튼 추가, 회귀 없음, `swift test` 1,070개 전량 통과. **S1 이후는 레고의 "실사용 3회 + 쓸 만하다" 판정 전까지 착수하지 않는다(게이트 필수, 우회 없음).**
- 사용자 결정: §Intent Reconciliation 참고(설계 문서) — S1 다음은 S3(대화) 먼저, S2(복습)는 그 다음 / 대화 크래시 대비 임시 저장 기본 ON.
...
| **S1** 코어+카드+퀴즈 | **S0 실사용 ≥3회 + "쓸 만하다" 판정 (필수, 우회 없음)** | S2·S3 무관 | S0 후 — **레고 판정 대기, 미착수** |
| **S3** 대화 | **S1 완료** | **S2와 무관 — S2 없이 착수 가능** | **레고 확정: S1 다음은 여기** |
| **S2** 복습 | **S1 완료** + 산출 노트 ≥10개 + 2주 실사용 | **S3와 무관** | **S3 다음** |
