...
- [ ] **RED**: `Tests/CmdMDTests/KordocRenderServiceTests.swift` 신설 — `KordocRenderService.wrapSVG(_:)`(가칭, static)에 대해:
...
- [ ] **GREEN**: `Sources/Services/KordocRenderService.swift` 신설.
...
- [ ] `swift build`·`swift test` 전체 통과 확인(950 + 신규 `wrapSVG` 테스트 개수).
- [ ] 커밋: "기능: hwpx 원본 렌더 서비스(KordocRenderService) — Task 1"

## Task 2: `HwpxRenderState` 모델

**목적:** 원본 렌더 상태를 담을 최소 모델. 순수 enum이라 별도 테스트 불필요(기존 `OfficeState`도 테스트 없음 — 동일 관례).

- [ ] `Sources/Models/HwpxRenderState.swift` 신설:
  ```swift
  ...
  ```
- [ ] `swift build` 통과 확인(신규 테스트 없음, 회귀 없음 확인만).
- [ ] 커밋: "기능: HwpxRenderState 모델 — Task 2"

## Task 3: `DocumentKind.kordocRenderableExtensions`

- [ ] `Sources/Models/DocumentKind.swift`에 `static let kordocRenderableExtensions: Set<String> = ["hwpx"]` 추가(기존 `nativelyRenderableOfficeExtensions` 주석 근처, 같은 스타일의 한국어 설명 주석 포함 — "kordoc render(SVG)로 원본 조판을 그릴 수 있는 확장자. HWPX 전용" 등).
- [ ] `swift test` 전체 통과 확인(회귀 없음).
- [ ] 커밋: "기능: DocumentKind에 kordocRenderableExtensions(hwpx) 추가 — Task 3"

## Task 4: `AppState` 배선

**목적:** 토글 가드를 확장하고, hwpx 렌더 로딩을 트리거하는 메서드를 추가한다. **기존 MS 오피스 분기·가드는 절대 삭제하지 말고 조건만 넓힌다.**

- [ ] `AppState`에 `var hwpxRenderStates: [UUID: HwpxRenderState] = [:]` 추가(다른 오피스 관련 상태 프로퍼티 근처).
- [ ] `KordocRenderService`를 `AppState`의 다른 kordoc 서비스들과 같은 자리에 프로퍼티로 보관(예: `private let kordocRenderService = KordocRenderService()`).
- [ ] `@MainActor func loadHwpxRender(tabID: UUID, fileURL: URL) async`:
...
- [ ] `toggleOfficeOriginalView(tabID:fileURL:)`의 가드를:
  ```swift
...
- [ ] 탭 종료/문서 교체 시 `hwpxRenderStates`도 정리하는 기존 정리 로직(탭 닫을 때 `officeStates`를 지우는 자리와 같은 곳)에 한 줄 추가.
- [ ] `swift test` 전체 통과 확인(회귀 없음 — 기존 `AppOfficeState` 계열 테스트가 있다면 그 스위트 특히 확인).
- [ ] 커밋: "기능: AppState에 hwpx 원본 렌더 로딩 배선 — Task 4"

## Task 5: `HwpxRenderPreview` 뷰

- [ ] `Sources/Views/HwpxRenderPreview.swift` 신설:
  ```swift
...
- [ ] `canShowOriginal` 계산 프로퍼티를:
  ...
  ```
  로 확장.
- [ ] 원본 보기 분기(`else if canShowOriginal && appState.officeShowingOriginal.contains(tabID)`) 안에서 확장자로 갈라짐:
  ```swift
  ...
  ```
- [ ] 토글 버튼 라벨·위치는 변경하지 않는다(기존 "원본 보기"/"글로 보기" 문구 그대로 — hwpx도 같은 버튼을 씀).
- [ ] `swift test` 전체 통과 확인(회귀 없음 — 기존 MS 오피스 원본 보기 스모크 시나리오가 코드 경로상 안 바뀌었는지 diff로 재확인).
- [ ] 커밋: "기능: OfficeReaderView에 hwpx 원본 보기 배선 — Task 6"

## Task 7: 전체 회귀 확인 + 재패키징·설치 + 수동 스모크 안내

- [ ] `swift test` 전체 재실행 — 기준선(950) 대비 정확히 신규 테스트 개수만큼 증가했는지, 실패 0인지 확인.
- [ ] `./scripts/package_app.sh` 재패키징 → `/Applications` 교체 설치.
- [ ] `CmdMD-fork_prd.md` §10 격차 5번 항목에 "hwpx 원본 보기 완료(2026-07-29), hwp는 도구 한계로 계속 보류" 갱신.
