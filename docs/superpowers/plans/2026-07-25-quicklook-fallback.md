...
- [x] `.pptx` 열어 슬라이드 넘김 확인(레고 실기 확인, 2026-07-28). `.zip`은 열면 아이콘만 뜨는 게 정상(macOS 자체가 zip 내용 미리보기 생성기를 안 갖고 있음 — 실제로 시스템 QuickLook 생성기 목록 확인함, 이 앱 문제 아님). `.key`·`.psd`는 미확인(해당 파일 없음).
- [ ] "○○으로 열기" 버튼이 실제 그 앱을 띄움
- [ ] ~~`.mdx` 열어 "글로 열기"로 편집기 전환~~ → 실제 구현은 다름: `.mdx`가 `QuickLookRouting.textFastPath`에 들어 있어(Task 1) 애초에 QuickLook을 거치지 않고 바로 글자(편집기)로 열림 — 버튼 자체가 없는 게 정상(2026-07-28 레고 확인: "글로 열기 버튼 안 나옴"은 이 설계 그대로임, 버그 아님). 체크리스트 문구가 낡음, 실제로는 더 나은 동작.
