...
- [ ] **수동 스모크(자동 불가) — 미확인.** 사용자가 릴리스 발행을 먼저 지시해 이 확인 없이 배포됨(2026-07-25). 재설치 후 다음을 눌러볼 것: 기본 위치 4개 표시·클릭 전환, 폴더 우클릭 등록/해제 토글, `⌥⌘M`+메뉴로 시트 호출·실제 이동+되돌리기, 목록 빈 상태에서 "다른 폴더로 이동…" 동작, 라이브러리 화면에서도 동일 동작, 기존 "폴더로 이동…"(다중 선택) 회귀 없음.
...
- **D+B**(작은 일, Phase 13) — v0.9.411로 GitHub 릴리스 발행 완료. **수동 스모크는 아직 미확인**(사용자가 릴리스 먼저 지시 — §6 Phase 13 참고).
...
**전역 단축키 파일 찾기 (Raycast식) — 완료(구현), 실기 확인 대기(2026-07-27)**
...
- **상태**: 사용자 승인("단축키 가자") 후 구현 완료. 계획과 달라진 점 하나 — **기본
  단축키를 ⌃⌘Space 대신 ⇧⌘Space로 바꿨다.** 조사해보니 ⌃⌘Space는 macOS 자체의 "이모지
  및 기호" 기본 단축키라 겹칠 위험이 컸다(맥이 이벤트를 먼저 가로채면 cmdALL엔 아예 안
  들어옴). 이미 문제없이 쓰고 있는 ⇧⌘M·⌘⇧8과 같은 Cmd+Shift 계열로 바꿔 그 위험을 없앴다.
  구현: `GlobalSearchOverlayController`(신규, 독립 `NSPanel` + `.nonactivatingPanel`로
  cmdALL을 활성화하지 않고 화면 중앙에 기존 `OmnisearchView` 재사용) + `AppDelegate`에
  전역·로컬 단축키 두 곳 배선(다른 앱 포커스 중·cmdALL 포커스 중 모두 반응) + 설정에
  켜고 끄기 토글(Tools 탭, 기본 ON) + 접근성 권한 안내 문구. 리매핑(다른 키 조합으로
  바꾸기)은 이번 범위에서 뺐다 — 기존 ⇧⌘M·⌘⇧8도 리매핑이 안 되는 것과 일관되게 고정
  조합으로 뒀다(과한 설계 방지). `swift build`·`swift test`(898개) 통과.
  **수동 스모크 대기** — 다른 앱을 보다가 ⇧⌘Space로 검색창이 뜨는지, 파일을 열면
  cmdALL이 앞으로 나오는지, Esc·바깥 클릭 시 원래 앱으로 조용히 돌아가는지 실기 확인 필요.
...
- **Docufinder에는 있는데 cmdALL엔 없는 것(구체적 격차 7가지, 우선순위 미정)**:
  ...
  4. 검색 결과 목록에서 다른 앱/웹으로 바로 드래그아웃(Shift+드래그=이동). cmdALL 라이브러리
     화면 드래그는 있지만 Omnisearch 결과 목록에서 되는지는 미확인.
  5. 문서를 마크다운 글자로 바꾸지 않고 원본 그대로의 조판·모양으로 보기(HWPX SVG 렌더·PDF
     원본 페이지·HWP 네이티브 렌더). cmdALL은 항상 kordoc→마크다운 변환 후 렌더.
  6. 스캔 PDF·사진 속 글자를 OCR로 읽어 검색 대상에 포함. cmdALL엔 없음.
  7. 이메일(.eml) 파일 검색(제목·보낸사람·받는사람·본문). cmdALL엔 없음.
  - 참고: Docufinder 자체엔 "다른 앱 쓰다가 단축키로 검색창 띄우기"(Raycast식 오버레이)
    기능은 없어 보임 — 그건 위 항목과는 별개로 사용자가 따로 요청한 아이디어.
- **상태**: 정보 정리만 완료, 우선순위·착수 여부 결정 대기 — "전역 단축키 진행" vs "이 7개
  중 먼저 고르기" 중 사용자 선택 대기.
