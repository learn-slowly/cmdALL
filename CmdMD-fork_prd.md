...
- **상태**: 사용자 승인("단축키 가자") 후 구현. 두 번 조합을 바꿨다 — ①처음 제안 ⌃⌘Space는
  ...
  방지). `swift build`·`swift test`(898개) 통과.
  **수동 스모크 대기** — 다른 앱을 보다가 ⌘⇧8로 검색창이 뜨는지, 파일을 열면
...
- **Docufinder에는 있는데 cmdALL엔 없는 것(구체적 격차 7가지, 우선순위 미정)**:
  ...
  5. **조사 결과 — 이번엔 보류 제안(2026-07-27)**. 문서를 마크다운 글자로 바꾸지 않고 원본
     그대로의 조판·모양으로 보기(HWPX SVG 렌더·PDF 원본 페이지·HWP 네이티브 렌더). **PDF는
     이미 해당**(§3.1, PDFKit 원본 페이지 뷰 — 격차 아님). 남은 건 HWP/HWPX/DOCX/XLSX인데,
     `npx kordoc --help`로 실제 확인한 결과 **kordoc은 `markdown`·`json` 출력만 지원하고
     SVG·HTML·PDF 같은 원본 조판 렌더 출력이 없다.** 대안은 LibreOffice 같은 무거운 의존성
     추가(PRD가 처음부터 피하려던 것, §4.1)뿐이라 지금 도구로는 만들 수 없다. **직접 만들지
     않고 사실만 보고 — 착수 안 함.**
  6. **조사 결과 — 범위 결정 필요(2026-07-27)**. 스캔 PDF·사진 속 글자를 OCR로 읽어 검색
     대상에 포함. macOS 내장 `Vision`(`VNRecognizeTextRequest`, 한국어 지원)으로 새 패키지
     의존성 0으로 만들 수 있다는 것까진 확인했다. **다만 성능 위험이 있다** — 지금 사용자
     컴퓨터의 백그라운드 폴더 훑기가 이미 16만개+ 파일을 처리 중인데, OCR은 파일 하나당
     지금의 글자 추출보다 훨씬 느리다(사진 한 장에 초 단위). 사진이 많은 폴더가 등록돼
     있으면 이미 오래 걸리는 훑기가 훨씬 더 느려질 수 있다. 범위를 좁혀야 한다(예: 스캔
     PDF만 OCR, 일반 사진 폴더는 자동 제외하고 켜고 끄는 스위치를 둔다 등) — **사용자가
     범위를 정해주면 그때 구현.** 착수 안 함.
  7. **완료(2026-07-27)** 이메일(.eml) 파일 검색(제목·보낸사람·받는사람·본문). 새
     ...
     `swift test` 전체 919개 통과.
- **상태**: 사용자가 "나열한 순서대로 진행" 결정(2026-07-27) — 2·3·4·7 완료. 5는 도구
  한계로 보류 제안, 6은 성능 위험 때문에 범위 결정 대기 — 사용자 선택 필요.
