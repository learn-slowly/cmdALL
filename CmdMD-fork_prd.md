...
**Phase 13: 빠른 파인더 — 기본 위치 + 빠른 이동(D+B)** — **코드 구현 완료(2026-07-25, §10)**, 수동 스모크·릴리스는 사용자 확인 대기. 계획 4단계 — ①`QuickMoveFolder` 모델+`AppState` 등록/해제/저장 ②단축키 `⌥⌘M` ③사이드바에 홈·데스크탑·다운로드·문서 고정 표시(D) ④폴더 우클릭 "빠른 이동 목록에 추가" 등록 토글+`QuickMoveSheet`+File 메뉴·단축키·우클릭 메뉴 배선(B). 이동 실행은 F1b가 만든 `performBatchMove`(로그+되돌리기)를 그대로 재사용 — 새 이동 로직 없음. 즐겨찾기와는 별개 저장소로 분리(2026-07-25 사용자 결정 — 자동 병합 대신 직접 등록 방식).
...
- [ ] **수동 스모크(자동 불가)** — 기본 위치 4개 표시·클릭 전환, 폴더 우클릭 등록/해제 토글, `⌥⌘M`+메뉴로 시트 호출·실제 이동+되돌리기, 목록 빈 상태에서 "다른 폴더로 이동…" 동작, 라이브러리 화면에서도 동일 동작, 기존 "폴더로 이동…"(다중 선택) 회귀 없음
- [ ] 원격(GitHub)에는 아직 안 올림 — 수동 스모크 통과 후 승인받고 진행
...
**"빠른 파인더" 로드맵 나머지 조각** — A·D+B 완료(D+B는 수동 스모크 대기, §6 Phase 13). 순서는 A → D+B → C → E.

- **D+B**(작은 일, Phase 13) — 코드 구현 완료, 수동 스모크·GitHub 릴리스 대기.
