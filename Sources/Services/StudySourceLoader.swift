...
/// office·이미지는 위치를 알 수 없으므로(`.unknown`) 기존 변환기(kordoc·`OCRService`)를
/// 그대로 재사용한다.
///
...
actor StudySourceLoader {
    ...
    func segments(for scope: StudyScope) async -> [StudySegment] {
        ...
        case .office:
            ...
            return trimmed.isEmpty ? [] : [StudySegment(text: trimmed, locator: .unknown)]
        case .image:
    ...
    private static func pageBounds(range: StudyScopeRange, pageCount: Int) -> (Int, Int)? {
        ...
        case .wholeFile:
            return (1, pageCount)
        case .lineRange:
            return nil // PDF는 줄 범위 개념이 없다 — 잘못 짝지어진 범위는 조각 0개.
        }
    }
    ...
    private static func lineBounds(range: StudyScopeRange, lineCount: Int) -> (Int, Int)? {
        ...
        case .wholeFile:
            return (1, lineCount)
        case .pageRange:
            return nil // 마크다운·텍스트는 쪽 범위 개념이 없다.
        }
    }
}
