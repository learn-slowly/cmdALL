...
extension AppState {
    ...
    // MARK: - 학습도우미 S0(2026-07-31) — 프롬프트 프리셋 실험

    /// "정리 카드 만들기" 프리셋 프롬프트를 문구입력창에 채우기만 한다(전송은 사용자가
    /// 기존 "질문" 버튼을 눌러야 실행). 새 파이프라인·파서 없이 기존 Claude 패널을 그대로
    /// 재사용하는 S0 실험 — 실사용 검증 전까지 이 이상 만들지 않는다.
    func fillStudyCardPrompt() {
        claudePrompt = """
        이 문서 내용을 바탕으로 한 장짜리 정리 카드를 만들어줘. 형식은 다음과 같이:
        ### [카드] 제목
        - 핵심 내용 1
        - 핵심 내용 2
        - 핵심 내용 3(있으면)
        > 근거: "원문에서 그대로 옮긴 문장"
        내용에 주제가 여러 개면 카드를 여러 개 만들어도 좋아. 원문에 없는 내용은 지어내지 마.
        """
    }

    /// "문제 뽑기" 프리셋 프롬프트를 문구입력창에 채우기만 한다. fillStudyCardPrompt()와
    /// 같은 이유로 프롬프트 채우기까지만 구현한다.
    func fillStudyQuizPrompt() {
        claudePrompt = """
        이 문서 내용을 바탕으로 연습 문제를 만들어줘. 형식은 다음과 같이:
        ### [문제 1] 문항 요지
        type: mcq
        Q: 질문 본문
        1) 보기 A
        ...
        해설: 왜 그 답인지
        > 근거: "원문에서 그대로 옮긴 문장"
        문제를 여러 개 만들어도 좋아. 원문에 없는 내용은 지어내지 마.
        """
    }
