...
/// AI가 짧게 답하는 자리(학습도우미 대화 S3·"Ask Claude" 패널)에서 마크다운을 실제로 그린다 —
/// AI 답이 흔히 쓰는 `**강조**`·목록·`` `코드` ``·인용·`###` 헤딩이 별표·해시 그대로 노출되지
/// 않게 한다(레고 2026-08-01 피드백: "마크다운 문법이 그대로 노출되는데?" — 처음엔 대화창만
/// 고쳤다가 "거기도"(Claude 패널) 요청으로 같은 날 확장). 문서 전체 미리보기가 쓰는
/// `MarkdownRenderer`(HTML+WKWebView, 표·수식·Mermaid·코드 하이라이트까지 지원)는 이런 자리처럼
/// 자주 바뀌는 짧은 텍스트엔 무겁다(웹뷰는 스트리밍 델타마다 재로드해야 하고, 대화창은 턴마다
/// 웹뷰가 여러 개 떠야 한다) — 그래서 같은 파서(swift-markdown, 이미 프로젝트 의존성)로 직접
/// SwiftUI 뷰를 그리는 훨씬 가벼운 네이티브 렌더러를 따로 둔다. 지원 범위는 AI 답변에 흔한
/// 것만(문단·헤딩·굵게·기울임·인라인 코드·코드 블록·인용·순서/비순서 목록) — 표·수식·Mermaid·
/// 링크 강조 등 문서 프리뷰 전용 기능은 의도적으로 생략(카드·문제 출력 계약 §O1~O3과도 무관).
///
